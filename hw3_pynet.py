...
bias
 ...
 #grad_input = grad_output * weight by chain rule
 grad_input = np.dot(grad_output, self.weight.T)
 #grad_weight = grad_output * input by chain rule
...
sqrt(2.0/(self.input_channel))).astype(np.float32)
 ...
 mean and running variance. (5 points)
 ...
 train -- bool, boolean indicator to specify the running mode, True for
training and False for testing
 ...
 output = ((input - mean)/(np.sqrt(variance+ self.eps))) * self.gamma +
self.beta
 #updating running mean and running variance
 self.r_mean = (1 - self.momentum) * mean + (self.momentum *
self.r_mean)
 self.r_var = (1 - self.momentum) * variance + (self.momentum *
self.r_var)
 elif train == False:
 #substituting running mean and variance for testing
 output = (input - self.r_mean)/(np.sqrt(self.r_var + self.eps)) *
self.gamma + self.beta
 ########################
 return output
 '''
 Backward computationg of batch normalization layer. (5 points)
 ...
 grad_input -- numpy array of shape (N, input_channel), gradient w.r.t
input
 grad_gamma -- numpy array of shape (input_channel), gradient w.r.t
gamma
 ...
 grad_sigma = np.sum((grad_xhat * (self.input - mean)), axis = 0) * -0.5 *
(variance + self.eps)**-1.5
 grad_mu = np.sum((grad_xhat * -1/(np.sqrt(variance + self.eps))), axis = 0)
+ grad_sigma * np.mean((-2 * (self.input - mean)), axis = 0)
 grad_input = (grad_xhat * 1/(np.sqrt(variance + self.eps))) + (grad_sigma *
2 * (self.input - mean)/m) + grad_mu/m
 ########################
...
class ReLU(object):
 ...
 pass
 '''
 Forward computation of ReLU. (5 points)
 You may want to save some intermediate variables to class membership
 (self.)
 Arguments:
 ...
 return output
 '''
 Backward computation of ReLU. (5 points)
 You can either modify grad_output in-place or create a copy.
 Arguments:
 grad_output -- numpy array having the same shape as input
 Output:
 grad_input -- numpy array has the same shape as grad_output. gradient
w.r.t input
 '''
 def backward(self, grad_output):
 ########################
 # TODO: YOUR CODE HERE #
 #here the gradient is is a unit function where 0 for negative numbers and 1
for anything greater than or equal to zero
...
class CrossEntropyLossWithSoftmax(object):
 ...
 pass
 '''
 Forward computation of cross entropy with softmax. (5 points)
 Tou may want to save some intermediate variables to class membership
 (self.)
 Arguments:
 input -- numpy array of shape (N, C), the prediction for each class,
where C is number of classes
 gt_label -- numpy array of shape (N), it is an integer array and the
value range from 0 to C-1 which
 specify the ground truth class for each input
 Output:
 output -- numpy array of shape (N), containing the cross entropy loss
on each input
 '''
 ...
 m = input.shape[0]
 p = softmax(input)
 #here to do the forward pass we take the negative log at the indices of the
gt label
 ...
 return output
 '''
 Backward computation of cross entropy with softmax. (5 points)
 ...
 Output:
 output -- numpy array of shape (N, C), the gradient w.r.t input of
forward function
 '''
 def backward(self, grad_output):
 ########################
 ...
 #the gradient is softmax(input) - ones
 #we calculate the ones at the shape of softmax where the ones are placed at
the gt_labels
 label_shape = p.shape
...
'''
 ...
 stride -- integer, stride of the sliding block in the spatial
dimension
 padding -- integer, zero padding on both size of inputs
 Returns:
 output_data -- numpy array of shape (N, (C * kernel_h * kernel_w), out_H,
out_W)
'''
...
'''
 ...
 Arguments:
 input_data -- numpy array of shape (N, (C * kernel_H * kernel_W), out_H,
out_W)
 kernel_h -- integer, height of the sliding blocks
 kernel_w -- integer, width of the sliding blocks
 stride -- integer, stride of the sliding block in the spatial
...
def col2im(input_data, kernel_h, kernel_w, stride=1, padding=0):
 ...
 img = np.pad(out,[(0, 0), (0, 0), (padding, padding), (padding, padding)],mode
= 'constant')
...
'''
class Conv2d(object):
 def __init__(self, input_channel, output_channel, kernel_size, padding = 0,
stride = 1):
 self.output_channel = output_channel
 self.input_channel = input_channel
 ...
 self.init_param()
 def init_param(self):
 self.weight = (np.random.randn(self.output_channel, self.input_channel,
self.kernel_h, self.kernel_w) * sqrt(2.0/(self.input_channel +
self.output_channel))).astype(np.float32)
 self.bias = np.zeros(self.output_channel).astype(np.float32)
 '''
 Forward computation of convolutional layer. (5 points)
 ...
 ########################
 # TODO: YOUR CODE HERE #
 #getting our image in the right dimension
 X_col = im2col(input, self.kernel_h, self.kernel_w, padding=self.padding,
stride=self.stride)
 self.X_col = X_col
 #initializing the dimensions
 N, mult, h_out, w_out = X_col.shape
 #reshaping the weight, image and bias matrices so we can broadcast and
conduct matrix multiplication
 ...
 return output
 '''
 Backward computation of convolutional layer. (5 points)
 ...
 Ouput:
 grad_input -- numpy array of shape(N, input_channel, H, W), gradient
w.r.t input
 grad_weight -- numpy array of shape(output_channel, input_channel,
kernel_h, kernel_w), gradient w.r.t weight
 grad_bias -- numpy array of shape(output_channel), gradient w.r.t
bias
 '''
 def backward(self, grad_output):
 ...
 X_col = im2col(self.input, self.kernel_h, self.kernel_w,
padding=self.padding, stride=self.stride)
 ...
 ow = np.sum(ow, axis = 1)
 grad_input = col2im(ow, self.kernel_h, self.kernel_w, self.stride,
self.padding)
 ########################
 return grad_input, grad_weight, grad_bias
'''
...
class MaxPool2d(object):
 ...
 self.stride = stride
 '''
 Forward computation of max pooling layer. (5 points)
 You should use im2col in this function. You may want to save some
 intermediate variables to class membership (self.)
 Arguments:
 input -- numpy array of shape (N, input_channel, H, W)
 ...
 #finding the maximum of that slixe
 output[i, c, j, k] = np.max(input_slice)
 self.input = input
 ########################
 return output
 '''
 Backward computation of max pooling layer. (5 points)
...
w.r.t input
 ...
 #similar porcess but we find the maximum where the slice is
equal to the maximum and multiplying by output
 grad_input[i, c, h_start:h_end, w_start:w_end] +=
np.multiply(kernel_slice == np.max(kernel_slice), grad_output[i,c,j,k])
 ########################
 return grad_input
